...
def load_cache(days):
  ...
  if os.path.exists(file_path):
    ...
    print('loaded data.....')
    return x, y, data['weaning_indexes'], data['length_of_cases'] 
    ...
    for file_name in file_list[:MAX_CASES]:  
          ...
          frames.append(df_x)   
          label_y.append(y)
          weaning_indexes.append(weaning_idx)
          length_of_cases.append(len(df))  
          length_of_mv_list.append(length_of_mv)
    ...
    result ['length_of_mv'] = np.repeat(np.array(length_of_mv_list), 120)
    ...
    np.savez_compressed(file_path, x=result.to_numpy(), y=np.array(label_y), 
                        weaning_indexes = np.array(weaning_indexes),
                        length_of_cases = np.array(length_of_cases),
                        length_of_mv = np.array(length_of_mv),
                        column_names=np.array(result.columns))
           
    return result, np.array(label_y), np.array(weaning_indexes), np.array(length_of_cases), np.array(length_of_mv)

def load_data(days=7):
     
    df,y,weaning_indexes,length_of_cases = load_cache(days) 
    ...
    x_cont = np.median(x_cont, axis=1)
